...
class Otp {
	...
	void calc() {
		OTPHash h = new OTPHash();

		if (this.sha == MD4) {
			this.hash = h.calcMD4(this.passphrase, this.seed, this.seq);
		} else if (this.sha == MD5) {
			this.hash = h.calcMD5(this.passphrase, this.seed, this.seq);
		} else {
			this.hash = h.calcSHA1(this.passphrase, this.seed, this.seq);
		}
	}
	...
	public static String btoe(final int index) {
